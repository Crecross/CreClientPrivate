...
namespace CreClientTest.Settings
{
    class Patches
    {
        private static readonly HarmonyLib.Harmony Instance = new HarmonyLib.Harmony("CreClient");
        ...
        public static void Init()
        {
            ...
            try
            {
                ...
                LoggerUtill.Log("[Patch] Analystics block", ConsoleColor.Green);
            }
            catch (Exception ex)
            {
            ...
            try
            {
                Instance.Patch(typeof(VRC.UI.Elements.QuickMenu).GetMethod("Awake"), new HarmonyMethod(AccessTools.Method(typeof(Main_BlazeAPI), nameof(Main_BlazeAPI.OnUIInit))));
            ...
            LoggerUtill.Log("[Patch] Patching Complete", ConsoleColor.Green);
        }
        ...
        private static void OnPlayerJoin(VRC.Player player)
        {
            ...
            if (player == PlayerWrapper.LocalPlayer())
                WorldWrapper.Init();
            for (int i = 0; i < Main_BlazeAPI.Instance.onPlayerJoinEventArray.Length; i++)
                Main_BlazeAPI.Instance.onPlayerJoinEventArray[i].OnPlayerJoin(player);
        ...
        private static void OnPlayerLeave(VRC.Player player)
        {
            ...
            for (int i = 0; i < Main_BlazeAPI.Instance.onPlayerLeaveEventArray.Length; i++)
                Main_BlazeAPI.Instance.onPlayerLeaveEventArray[i].PlayerLeave(player);
        ...
        private static bool OnAvatarAssetBundleLoad(ref UnityEngine.Object __0)
        {
            ...
            for (int i = 0; i < Main_BlazeAPI.Instance.OnAssetBundleLoadEventArray.Length; i++)
                if (!Main_BlazeAPI.Instance.OnAssetBundleLoadEventArray[i].OnAvatarAssetBundleLoad(gameObject, avatarId))
                    return false;
        ...
        {
            for (int i = 0; i < Main_BlazeAPI.Instance.onUdonEventArray.Length; i++)
                if (!Main_BlazeAPI.Instance.onUdonEventArray[i].OnUdon(__0, __1))
                    return false;
            return true;
        }
        ...
        private static bool OnEventPatch(EventData __0)
        {
            ...
            for (int i = 0; i < Main_BlazeAPI.Instance.onEventEventArray.Length; i++)
                if (!Main_BlazeAPI.Instance.onEventEventArray[i].OnEvent(__0))
                    return false;
            return true;
        }
        ...
        {
            for (int i = 0; i < Main_BlazeAPI.Instance.onSendOPEventArray.Length; i++)
                if (!Main_BlazeAPI.Instance.onSendOPEventArray[i].OnSendOP(__0, ref __1, ref __2))
                    return false;
        ...
        {
            for (int i = 0; i < Main_BlazeAPI.Instance.onRPCEventArray.Length; i++)
                if (!Main_BlazeAPI.Instance.onRPCEventArray[i].OnRPC(__0, __1, __2, __3, __4))
                    return false;
